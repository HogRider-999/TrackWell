...
const dict = {
    ...
    'en': {
        ...
        quotes: [
            ...
            "The only shortcut to success is to work harder.",
            "Your rivals are working hard; what's your excuse?",
            "Muscle is the armor of the soul.",
            "Every effort will be rewarded one day.",
            "Walk today, or you'll have to run tomorrow.",
...
let currentScannedFood = null, waterIntake = 0, currentWaterCups = 0;
let currentCountdown = { name: "2026 統測倒數", date: "2026-04-25" };
let todos = JSON.parse(localStorage.getItem('trackwell_todos')) || [];
let tempCustomExercises = [], selectedPlanIcon = './Leg day.png';
let exercisesData = JSON.parse(localStorage.getItem('trackwell_workouts')) || {};
let currentViewingPlan = null;
let pomoInterval, pomoTimeLeft = 25 * 60, pomoTotalTime = 25 * 60, pomoIsRunning = false;
let customTimerInterval, customTimeLeft = 0, customIsRunning = false;
let longPressTimer;

const weightMockData = {
    '3years': [
        { label: '2024', val: 55.0 }, { label: '2025', val: 58.5 }, { label: '2026', val: 61.2 }
    ],
    'year': [
        { label: 'Jan', val: 60.5 }, { label: 'Feb', val: 61.8 }, { label: 'Mar', val: 62.0 }
    ],
    '3months': [
        { label: '12月', val: 59.5 }, { label: '1月', val: 61.0 }, { label: '今天', val: 62.0, isToday: true }
    ]
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadCountdown(); updateExamCountdown(); initWaterCups(); renderTodos(); renderWorkoutCards();
...
function showPage(id, navItem) {
    ...
    document.getElementById(id).classList.add('active'); if(navItem) navItem.classList.add('active');
...
function closeModal(e, id) { if(!e || e.target === e.currentTarget || e.target.classList.contains('close-btn') || e.target.classList.contains('btn-cancel')) { document.getElementById(id).style.display = 'none'; } }
...
function saveStepsSettings() {
    ...
    closeModal(null, 'steps-settings-modal');
}
...
function saveCalSettings() {
    ...
    closeModal(null, 'cal-goal-modal');
}
...
function saveActivity() {
    ...
    updateStats(0, 0, 0, 0);
    closeModal(null, 'activity-input-modal');
}

function renderWorkoutCards() {
    const grid = document.getElementById('workout-card-grid'); grid.innerHTML = '';
    ...
    Object.keys(exercisesData).forEach(title => {
        const plan = exercisesData[title]; const card = document.createElement('div'); card.className = 'workout-album-lg';
    ...
    const addBtn = document.createElement('div'); addBtn.className = 'workout-album-lg add-btn-card'; addBtn.onclick = () => openModal('custom-plan-modal');
...
function openDay(title, iconUrl) {
    ...
    document.getElementById('workout-home').style.display = 'none'; document.getElementById('workout-detail').style.display = 'block';
    document.getElementById('current-day-title').innerText = title; const imgEl = document.getElementById('detail-cover');
    imgEl.src = iconUrl; imgEl.onerror = function() { this.src = './custom.jpg'; };
    renderDayExercises(title);
}

function backToWorkoutHome() {
    currentViewingPlan = null;
    document.getElementById('workout-home').style.display = 'block'; document.getElementById('workout-detail').style.display = 'none';
}

function renderDayExercises(title) {
    const container = document.getElementById('exercise-list-container'); container.innerHTML = '';
    const plan = exercisesData[title]; if(!plan) return;
    ...
    if(exercises.length === 0) {
        ...
        exercises.forEach((ex, i) => {
            ...
            row.innerHTML = `
                ...
                <div class="info" style="cursor:pointer;" onclick="toggleExComplete(this)"><strong style="font-size:16px; color:#fff;">${ex.name}</strong><br><small style="color:#888; font-weight:600;">${ex.desc}</small></div>
                <div style="font-weight:bold; font-size:14px; color:#fff; cursor:pointer;" onclick="toggleExComplete(this.previousElementSibling)">${ex.meta}</div>
    ...
    const addArea = document.createElement('div'); addArea.className = 'add-ex-form detail-add-form';
    addArea.innerHTML = `
...
function toggleExComplete(el) {
    const row = el.parentElement; row.classList.toggle('completed');
    const icon = row.querySelector('.check-icon');
    if(row.classList.contains('completed')) { icon.className = 'fas fa-check-circle check-icon'; icon.style.color = 'var(--primary)'; } 
    else { icon.className = 'far fa-circle check-icon'; icon.style.color = '#555'; }
}
...
function deleteExFromDay(title, index) {
    ...
    renderDayExercises(title); renderWorkoutCards();
}

function addExToDay(title) {
    const n = document.getElementById('detail-ex-name').value, m = document.getElementById('detail-ex-meta').value, d = document.getElementById('detail-ex-desc').value;
    if(!n) return;
...
function deleteEntirePlan(title) {
    ...
    if(confirm(dict[lang].wConfirmDel)) {
        delete exercisesData[title]; localStorage.setItem('trackwell_workouts', JSON.stringify(exercisesData));
        backToWorkoutHome(); renderWorkoutCards();
    }
}

function selectPresetIcon(el, url) { document.querySelectorAll('.preset-icon').forEach(i => i.classList.remove('active')); el.classList.add('active'); selectedPlanIcon = url; }
function previewPlanIcon(e) { const f = e.target.files[0]; if(f) { const reader = new FileReader(); reader.onload = () => { selectedPlanIcon = reader.result; document.querySelectorAll('.preset-icon').forEach(i => i.classList.remove('active')); document.querySelector('.local-icon-upload').style.borderColor = 'var(--primary)'; }; reader.readAsDataURL(f); } }
function addCustomExToList() {
    const n = document.getElementById('ex-name').value, m = document.getElementById('ex-meta').value, d = document.getElementById('ex-desc').value;
    if(!n) return; tempCustomExercises.push({ name: n, meta: m, desc: d });
    document.getElementById('ex-name').value = ''; document.getElementById('ex-meta').value = ''; document.getElementById('ex-desc').value = ''; renderCustomExList();
}
function removeCustomEx(idx) { tempCustomExercises.splice(idx, 1); renderCustomExList(); }
function renderCustomExList() { document.getElementById('custom-ex-list').innerHTML = tempCustomExercises.map((ex, i) => `<div class="list-row" style="padding:10px; margin-bottom:5px; background:#222;"><span style="color:#fff; font-weight:bold; font-size:14px;">${ex.name} <small style="color:#888;">(${ex.meta})</small></span><i class="fas fa-times" onclick="removeCustomEx(${i})" style="color:#ff5252; cursor:pointer;"></i></div>`).join(''); }
function saveCustomPlan() {
    const title = document.getElementById('plan-name').value; if (!title) return;
    exercisesData[title] = { icon: selectedPlanIcon, exercises: [...tempCustomExercises] };
    localStorage.setItem('trackwell_workouts', JSON.stringify(exercisesData));
    tempCustomExercises = []; renderCustomExList(); document.getElementById('plan-name').value = ''; selectedPlanIcon = './Leg day.png';
    closeModal(null, 'custom-plan-modal'); renderWorkoutCards();
}

function saveBodyStats() {
    const w = document.getElementById('input-weight').value, f = document.getElementById('input-fat').value, m = document.getElementById('input-muscle').value;
    ...
    closeModal(null, 'body-input-modal');
}
...
function updateBodyDisplay(data) {
    ...
    if(topStats) topStats.innerHTML = `160cm | ${data.weight}kg`;
...
function updateChart(range) {
    ...
    buttons.forEach(btn => { btn.classList.toggle('active', btn.getAttribute('onclick').includes(range)); });
    container.innerHTML = '';
    ...
    dataSet.forEach(item => {
        ...
        barGroup.innerHTML = `<div class="bar-val-tooltip">${item.val} kg</div><div class="bar-fill" style="height: ${heightPercentage}%; ${todayStyle}"></div><div class="bar-label">${item.label}</div>`;
        container.appendChild(barGroup);
    });
}

function loadCountdown() { const saved = localStorage.getItem('trackwell_countdown'); if(saved) { currentCountdown = JSON.parse(saved); document.getElementById('side-countdown-label').innerText = currentCountdown.name; } }
function saveCountdownSettings() {
    const name = document.getElementById('custom-event-name').value, date = document.getElementById('custom-event-date').value;
    if(name && date) { currentCountdown = { name, date }; localStorage.setItem('trackwell_countdown', JSON.stringify(currentCountdown)); document.getElementById('side-countdown-label').innerText = name; updateExamCountdown(); closeModal(null, 'countdown-settings-modal'); }
}
function updateExamCountdown() { const diff = new Date(currentCountdown.date + 'T00:00:00') - new Date(); document.getElementById('exam-days').innerText = Math.max(0, Math.ceil(diff / (1000 * 60 * 60 * 24))); }

function setTimerMode(minutes) { resetPomodoro(); pomoTimeLeft = minutes * 60; pomoTotalTime = minutes * 60; updatePomoUI(); document.querySelectorAll('.sm-mode').forEach(btn => { btn.classList.remove('active'); if (parseInt(btn.innerText) === minutes) btn.classList.add('active'); }); }
function togglePomodoro() {
    const btn = document.getElementById('pomo-play-btn');
    if (pomoIsRunning) { clearInterval(pomoInterval); btn.innerHTML = '<i class="fas fa-play"></i>'; } 
    else { pomoInterval = setInterval(() => { if (pomoTimeLeft <= 0) { clearInterval(pomoInterval); alert("專注時間結束！"); resetPomodoro(); return; } pomoTimeLeft--; updatePomoUI(); }, 1000); btn.innerHTML = '<i class="fas fa-pause"></i>'; }
    pomoIsRunning = !pomoIsRunning;
}
function updatePomoUI() { const m = Math.floor(pomoTimeLeft / 60), s = pomoTimeLeft % 60; document.getElementById('main-timer').innerText = `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`; document.getElementById('pomo-progress').style.strokeDashoffset = 283 - (pomoTimeLeft / pomoTotalTime) * 283; }
function resetPomodoro() { clearInterval(pomoInterval); pomoIsRunning = false; pomoTimeLeft = pomoTotalTime; document.getElementById('pomo-play-btn').innerHTML = '<i class="fas fa-play"></i>'; updatePomoUI(); }

function toggleCustomTimer() {
    const btn = document.getElementById('ct-start-text');
    const lang = document.getElementById('lang-select').value || 'zh';
    if(customIsRunning) { clearInterval(customTimerInterval); btn.innerText = lang === 'en' ? 'Resume' : '繼續'; } 
    else {
        if(customTimeLeft === 0) customTimeLeft = (parseInt(document.getElementById('ct-h').value) || 0) * 3600 + (parseInt(document.getElementById('ct-m').value) || 0) * 60 + (parseInt(document.getElementById('ct-s').value) || 0);
        if(customTimeLeft <= 0) return; document.getElementById('ct-inputs').style.display = 'none';
        customTimerInterval = setInterval(() => { if(customTimeLeft <= 0) { clearInterval(customTimerInterval); alert("計時結束！"); resetCustomTimer(); return; } customTimeLeft--; updateCustomTimerUI(); }, 1000);
        btn.innerText = lang === 'en' ? 'Pause' : '暫停';
    }
    customIsRunning = !customIsRunning;
}
function updateCustomTimerUI() { const h = Math.floor(customTimeLeft / 3600), m = Math.floor((customTimeLeft % 3600) / 60), s = customTimeLeft % 60; document.getElementById('custom-timer-display').innerText = `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`; }
function resetCustomTimer() { clearInterval(customTimerInterval); customIsRunning = false; customTimeLeft = 0; const lang = document.getElementById('lang-select').value || 'zh'; const startText = document.getElementById('ct-start-text'); if(startText) startText.innerText = dict[lang].ctStart; document.getElementById('custom-timer-display').innerText = "00:00:00"; document.getElementById('ct-inputs').style.display = 'flex'; }
...
function addTodo() { const input = document.getElementById('todo-input'); if(!input.value) return; todos.push({ text: input.value, done: false }); input.value = ''; saveTodos(); renderTodos(); }
function toggleTodo(index) { todos[index].done = !todos[index].done; saveTodos(); renderTodos(); }
function deleteTodo(index) { todos.splice(index, 1); saveTodos(); renderTodos(); }
function saveTodos() { localStorage.setItem('trackwell_todos', JSON.stringify(todos)); }
function renderTodos() { document.getElementById('todo-list').innerHTML = todos.map((t, i) => `<li class="todo-item ${t.done ? 'done' : ''}"><i class="${t.done ? 'fas fa-check-circle' : 'far fa-circle'}" onclick="toggleTodo(${i})"></i><span>${t.text}</span><i class="fas fa-trash" onclick="deleteTodo(${i})"></i></li>`).join(''); }

function handleWaterClick(event, element) { changeWater((event.clientX - element.getBoundingClientRect().left) < element.getBoundingClientRect().width / 2 ? -1 : 1); }
function changeWater(amount) { currentWaterCups = Math.max(0, Math.min(8, currentWaterCups + amount)); toggleWater(currentWaterCups); }
function initWaterCups() { document.getElementById('water-cups-container').innerHTML = Array(8).fill('<div class="cup-container"><div class="cup-water"></div></div>').join(''); }
function toggleWater(cupNumber) { document.querySelectorAll('.cup-container').forEach((cup, idx) => cup.classList.toggle('active', idx < cupNumber)); waterIntake = cupNumber * 312.5; document.getElementById('water-count-txt').innerText = `${Math.round(waterIntake)} / 2500 ml`; }

function updateStats(kcal, c, p, f) {
    dietState.intake += kcal; dietState.carbs += c; dietState.protein += p; dietState.fat += f;
...
function saveManualFood() { addFoodItem(document.getElementById('manual-name').value || "手動紀錄", parseInt(document.getElementById('manual-kcal').value)||0, parseInt(document.getElementById('manual-carbs').value)||0, parseInt(document.getElementById('manual-protein').value)||0, parseInt(document.getElementById('manual-fat').value)||0, "手動"); closeModal(null, 'manual-food-modal'); }
function startAIScan() {
    document.getElementById('scanner-laser').style.display = 'block'; document.getElementById('ai-result-area').style.display = 'block'; document.getElementById('ai-analyze-btn').style.display = 'none';
    const status = document.getElementById('scan-status'); const lang = document.getElementById('lang-select').value || 'zh';
    status.innerText = lang === 'en' ? "Loading model..." : "載入模型...";
    setTimeout(() => { 
        status.innerText = lang === 'en' ? "Analyzing..." : "分析中..."; 
        setTimeout(() => { 
            document.getElementById('scanner-laser').style.display = 'none'; status.innerText = "✅ 完成"; status.style.color = "#4caf50"; 
...
function addScannedFood() { if(currentScannedFood) addFoodItem(currentScannedFood.name, currentScannedFood.kcal, currentScannedFood.carbs, currentScannedFood.protein, currentScannedFood.fat, "AI"); closeModal(null, 'ai-scan-modal'); }
function addFoodItem(name, kcal, c, p, f, type) {
    const item = document.createElement('div'); item.className = 'food-item';
    item.innerHTML = `<div><strong style="font-size:16px; color:#fff;">${name}</strong><small style="color:var(--primary); font-weight:bold; margin-left:8px;">[${type}]</small><br><span style="font-size:12px; color:#888; font-weight:bold;">碳水 ${c}g | 蛋白 ${p}g | 脂肪 ${f}g</span></div><div style="font-size:20px; font-weight:900; color:#fff;">${kcal} <small style="color:#666; font-size:12px;">kcal</small></div>`;
    document.getElementById('lunch-list').appendChild(item); updateStats(kcal, c, p, f);
}

function previewImage(event) { const file = event.target.files[0]; if(file) { const reader = new FileReader(); reader.onload = () => { document.getElementById('upload-placeholder').style.display='none'; document.getElementById('food-preview').src = reader.result; document.getElementById('food-preview').style.display='block'; document.getElementById('ai-analyze-btn').disabled = false; }; reader.readAsDataURL(file); } }